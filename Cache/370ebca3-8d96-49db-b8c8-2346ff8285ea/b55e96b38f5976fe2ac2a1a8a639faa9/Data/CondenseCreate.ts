...
export class CondenseCreate extends BaseScriptComponent {
    ...
    onStart() {
        ...
        print("CondenseCreate ready on node: " + this.nodeId);
    ...
    private openNode() {
        ...
        if (!node) {
            print("ERROR: No tree data found for node: " + this.nodeId);
            return;
        }
    ...
    private closeNode() {
        for (var i = this.getSceneObject().getChildrenCount() - 1; i >= 0; i--) {
            ...
            if (child.name.indexOf("GeneratedNode_") === 0) {
                print("Deleting child branch: " + child.name);
                child.destroy();
            }
        }
    }
