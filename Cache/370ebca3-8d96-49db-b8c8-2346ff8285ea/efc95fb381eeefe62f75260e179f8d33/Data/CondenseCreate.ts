...
export class CondenseCreate extends BaseScriptComponent {
    ...
    private openNode(parentUid: string, nodeId: string, parentObject: SceneObject) {
        ...
        for (var i = 0; i < count; i++) {
            ...
            this.bindGeneratedNode(newBox, childUid, childNodeId);
    ...
    private bindGeneratedNode(nodeObject: SceneObject, uid: string, nodeId: string) {
        var interactable = nodeObject.getComponent(
            Interactable.getTypeName()
        ) as unknown as Interactable;

        if (!interactable) {
            print("ERROR: Generated node has no Interactable: " + nodeObject.name);
            return;
        }
