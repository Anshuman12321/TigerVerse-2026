...
let TreeManager = (() => {
    ...
    var TreeManager = _classThis = class extends _classSuper {
        ...
        onStart() {
            ...
            this.bindClick(this.rootBox, this.rootUid, this.fileTree);
            print("TREE MANAGER READY");
        }
        bindClick(obj, uid, data) {
            var interactable = obj.getComponent(Interactable_1.Interactable.getTypeName());
            if (!interactable) {
                print("ERROR: No Interactable found on " + obj.name);
                return;
            }
        ...
        bindClickDelayed(obj, uid, data) {
            var delay = this.createEvent("DelayedCallbackEvent");
            delay.bind(() => {
                this.bindClick(obj, uid, data);
            });
            delay.reset(0.05);
        }
        closeNode(uid) {
