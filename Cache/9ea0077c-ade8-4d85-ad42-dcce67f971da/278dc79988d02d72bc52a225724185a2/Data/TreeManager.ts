...
export class TreeManager extends BaseScriptComponent {
    ...
    public onGeneratedNodeClicked(nodeObject: SceneObject) {
        var uid = this.getUidFromName(nodeObject.name);
        var nodeData = this.createdNodes[uid];

        if (!nodeData) {
            print("ERROR: No node data found for " + nodeObject.name);
            return;
        }

        this.onNodeClicked(uid, nodeData.data, nodeObject);
    }
    ...
    private openNode(parentUid: string, parentData: FileNode, parentObject: SceneObject) {
        ...
        for (var i = 0; i < count; i++) {
            ...
            print("CREATED: " + childData.label + " uid=" + childUid);
        }
