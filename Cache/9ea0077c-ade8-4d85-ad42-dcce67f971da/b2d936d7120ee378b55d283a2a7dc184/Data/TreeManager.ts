...
export class TreeManager extends BaseScriptComponent {
    ...
    @input horizontalSpacing: number = 10;
    @input forwardSpacingZ: number = -10;
    @input heightOffsetY: number = 0.5;
    ...
    private buildTree() {
        ...
        this.bindClickToNode(this.rootBox, rootPath);
    ...
    private createChildrenRecursive(
        ...
        for (var i = 0; i < count; i++) {
            ...
            this.bindClickToNode(box, childPath);
    ...
    private bindClickToNode(obj: SceneObject, nodePath: string) {
        var interactable = this.findInteractableOnObject(obj);

        if (!interactable) {
            print("NO INTERACTABLE FOUND ON " + obj.name);
            return;
        }

        var savedPath = nodePath;

        if (interactable.onTriggerStart) {
            interactable.onTriggerStart.add(() => {
                print("CLICKED NODE: " + savedPath);
                this.onNodeClicked(savedPath);
            });

            print("BOUND CLICK: " + savedPath);
            return;
        }
        ...
        print("INTERACTABLE HAS NO TRIGGER EVENT ON " + obj.name);
    }

    private findInteractableOnObject(obj: SceneObject): any {
        var components = obj.getComponents("Component.ScriptComponent");
