...
// and ensure it has a Collider or MeshVisual with collision enabled for hit testing.
...
function hitTestSquare(x, y) {
    // Perform a simple point-inside test using screen-space bounds of the object, if available.
    // If your project exposes a more precise raycast/hitTest API, replace this with that call.
    if (!squareButtonBox3D || !squareButtonBox3D.getScreenTransform) { return false; }
    var st = squareButtonBox3D.getScreenTransform();
    if (!st) { return false; }
    // Convert object's rect to normalized screen bounds
    var min = st.topLeft;
...
// Intercept touches that begin on the 3D button to avoid starting a drag
...
script.touch_events.onTouchUp.add(function(touchId, touchX, touchY) {
    if (pressBeganOnButton && touchId === activePressTouchId) {
        ...
        // Treat as a tap only if touch ends on the same button
        if (isOverButton) {
